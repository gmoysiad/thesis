...
from tslearn.cycc import y_shifted_sbd_vec
...
class SAND():
	...
	def fit(self, X, y=None, online=False, alpha=None, init_length=None, batch_size=None, overlaping_rate=10,
			verbose=False):
	...
	def _set_initial_S(self, X, idx, cluster_centers):
		...
		sz = X.shape[1]
		Xp = y_shifted_sbd_vec(cluster_centers, X, norm_ref=-1, norms_dataset=np.linalg.norm(X, axis=(1, 2)))
	...
	# Compute mean distance of an element in a cluster
